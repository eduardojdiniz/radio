...
from ..unpaired_dataset import MRISliceUnpairedDataset
from ..gan_queue import GANQueue

__all__ = ["MRI3T27TPatchNoQueueDataModule"]


class MRI3T27TPatchNoQueueDataModule(MRI3T27TDataModule):
    """
    ...
    dataset_cls = MRISliceUnpairedDataset
    ...
    def save(self,
        ...
        for dl_idx, (batch_a, batch_b) in tqdm(enumerate(dataloader),
                                               total=n_subjects /
                                               dataloader.batch_size):
            for b_idx in range(batch_size):
                img_idx = dl_idx * batch_size + b_idx
                image_a = batch_a[b_idx]
                image_b = batch_b[b_idx]
                save_root_a = ensure_exists(
                    Path(root).expanduser() / self.study / subj_dir /
                    f'{fold}_3T_MPR' / data_dir)
                save_root_b = ensure_exists(
                    Path(root).expanduser() / self.study / subj_dir /
                    f'{fold}_7T_MPR' / data_dir)
                filename = f'{img_idx}.png'
                img_slice_a = rotate(image_a.numpy(), radiological=True)
                img_slice_a = ((img_slice_a * 127.5) + 127.5).astype(np.uint8)
                plt.imsave(
                    str(save_root_a / filename),
                    img_slice_a,
                    cmap='gray',
                )
                img_slice_b = rotate(image_b.numpy(), radiological=True)
                img_slice_b = ((img_slice_b * 127.5) + 127.5).astype(np.uint8)
                plt.imsave(
                    str(save_root_b / filename),
                    img_slice_b,
                    cmap='gray',
                )
                plt.cla()
                plt.clf()
                plt.close()
                del image_a, image_b, img_slice_a, img_slice_b
                gc.collect()
            del batch_a, batch_b
            gc.collect()

...
__all__ = ["UnpairedDataset", "MRIUnpairedDataset"]
...
class UnpairedDataset(FolderDataset):
    ...
    def __init__(
        ...
        self.samples_dict = self.get_samples(stage)
        self._subjects_a = self.samples_dict[self.domain_a]
        self._subjects_b = self.samples_dict[self.domain_b]
        self.size_a = len(self.samples_dict[self.domain_a])
        self.size_b = len(self.samples_dict[self.domain_b])
    ...
    def __getitem__(self, idx: int) -> PairedSample:
        ...
        # path_a, target_a = self.samples_dict[self.domain_a][idx % self.size_a]
        # path_b, target_b = self.samples_dict[self.domain_b][idx % self.size_b]
        # sample_a, sample_b = self.loader(path_a), self.loader(path_b)
        sample_a, target_a = self.samples_dict[self.domain_a][idx %
                                                              self.size_a]
        sample_b, target_b = self.samples_dict[self.domain_b][idx %
                                                              self.size_b]
...
class MRIUnpairedDataset(UnpairedDataset):
    ...
    def get_max_shape(self) -> Tuple[int, int, int]:
        ...
        """
        shapes_a = [
            image.spatial_shape for subject in self.samples_dict[self.domain_a]
            for image in self.loader(subject[0]).get_images()
        ]

        shapes_b = [
            image.spatial_shape for subject in self.samples_dict[self.domain_b]
            for image in self.loader(subject[0]).get_images()
        ]
    ...
    def get_sampling_map(
        ...
        """
        probabilities = create_probability_map(subject, patch_size)
